...
RUN \
    cd /apps \
    && wget https://github.com/xpack-dev-tools/arm-none-eabi-gcc-xpack/releases/download/v13.2.1-1.1/xpack-arm-none-eabi-gcc-13.2.1-1.1-linux-arm64.tar.gz -O gcc-arm-none-eabi.tar.gz \
...
RUN \
    cd /apps \
    && curl -d "accept_license_agreement=accepted&submit=Download+software" -X POST -O "https://www.segger.com/downloads/jlink/JLink_Linux_V810g_arm64.deb" 
...
RUN \
    cd /apps \
    && dpkg --unpack JLink_Linux_V810g_arm64.deb \
